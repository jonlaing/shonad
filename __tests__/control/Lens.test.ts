...
describe("Lens", () => {
  ...
  describe("makeLensHelper", () => {
    ...
    const helper = L.makeDictHelper<A>(fallback);
    const dict_ = helper(Maybe.just(dict));

    it("gets a top level value", () => {
      expect(dict_.a()).toBe(1);
    });

    it("gets a top level default value", () => {
      expect(dict_.h()).toBe(0);
    });

    it("gets a list", () => {
      expect(dict_.d()).toStrictEqual([1, 2, 3]);
    });

    it("gets a nested value", () => {
      expect(dict_.f().g()).toBe(2);
    });

    it("gets a nested default", () => {
      expect(dict_.i().j()).toBe(0);
    });

    it("gets a nested list", () => {
      expect(dict_.f().m()).toStrictEqual([5, 6]);
    });

    it("gets a default nested list", () => {
      expect(dict_.i().l()).toStrictEqual([]);
    });
  });
});
